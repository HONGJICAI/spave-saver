...
impl PluginManager {
    ...
    pub fn process_file(&self, source: &Path, output_dir: &Path, plugin_orders: Option<&[String]>) -> Result<CompressionResult> {
        ...
        let plugin = self.find_plugin(source)?
    ...
    pub fn process_batch(
        ...
        let results: Vec<Result<CompressionResult>> = sources.iter()
...
static GLOBAL_PLUGIN_MANAGER: Lazy<Arc<RwLock<PluginManager>>> = Lazy::new(|| {
    ...
    use crate::plugins::{WebPConverterPlugin, ImageZipToWebpZipPlugin, AnimatedWebPConverterPlugin};
...
pub fn init_plugin_manager_with(plugins: Vec<Box<dyn CompressionPlugin>>) -> Arc<RwLock<PluginManager>> {
    let mut manager = PluginManager::new();
...
mod tests {
    ...
    impl CompressionPlugin for MockPlugin {
        ...
        fn can_handle(&self, path: &Path) -> Result<(bool, Option<String>)> {
            let can_handle = has_extension(path, &self.extensions.iter()
                .map(|s| s.as_str())
                .collect::<Vec<_>>());
            let reason = if can_handle {
                Some(format!("File extension matches supported extensions"))
            } else {
                Some(format!("File extension not supported by this plugin"))
            };
    ...
    fn test_custom_plugin_manager_for_testing() {
        ...
        let mock_plugins: Vec<Box<dyn CompressionPlugin>> = vec![
            Box::new(MockPlugin {
    ...
    fn test_plugin_orders() {
        ...
        let result = manager.process_file(path, &temp_dir, Some(&orders)).unwrap();
